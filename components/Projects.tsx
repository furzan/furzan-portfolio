...
const gridClasses: Record<string, string> = {
  ...
  f: "md:col-start-2 md:row-start-3 md:col-span-1 md:row-span-1",
  g: "md:col-start-3 md:row-start-3 md:col-span-1 md:row-span-1",
};
      ...
      <div className="relative z-10 mx-auto max-w-[1800px] px-4 sm:px-10 lg:px-16">
        <ScrollReveal>
          <div className="glass-panel max-w-2xl p-6 sm:p-8">
            ...
            <p className="mt-3 font-body text-body-sm leading-relaxed text-[#e6d8c3]">
              A mix of agentic AI systems and full-stack builds — pulled
              straight from what&apos;s shipped and public on GitHub.
            </p>
          </div>
        </ScrollReveal>
        ...
        <div className="mt-10 grid grid-cols-1 gap-5 md:grid-cols-3 md:auto-rows-[14.5rem]">
          {projects.map((project, i) => {
            const isFeatured = project.area === "a";
            return (
              <ScrollReveal
                key={project.title}
                variant="scale-up"
          ...
          <ScrollReveal
            ...
            className={gridClasses.g}
          >
            <a
              href={profile.github}
              target="_blank"
              rel="noreferrer"
              className="group flex min-h-[10rem] md:min-h-0 h-full flex-col items-start justify-between glass-panel p-card border-dashed border-[#d4b483]/60 hover:border-paper hover:-translate-y-1"
            >
              <Github className="h-5 w-5 text-paper" />
              <div>
                <p className="font-display text-heading-2xs italic text-paper">
                  22+ repos
                </p>
                <p className="mt-1 flex items-center gap-1 font-body text-caption text-[#d4b483]">
                  View all on GitHub
